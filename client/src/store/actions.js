...
export const productActions = {
  ...
  productById({ commit }, payload) {
    ...
    return axios.get(`${API_BASE}/products/${productId}`).then(response => {
      commit(PRODUCT_BY_ID_SUCCESS, { product: response.data });
    });
  },
...
export const manufacturerActions = {
  ...
  updateManufacturer({ commit }, payload) {
    ...
    axios
      ...
      .then(() => {
        commit(UPDATE_MANUFACTURER_SUCCESS, payload);
        Message({
          message: "制造商更新成功！",
          type: "success"
        });
  ...
  manufacturerById({ commit }, { manufacturerId }) {
    commit(MANUFACTURER_BY_ID);
    return axios
      .get(`${API_BASE}/manufacturers/${manufacturerId}`)
      .then(response => {
        commit(MANUFACTURER_BY_ID_SUCCESS, { manufacturer: response.data });
      });
  }
};
